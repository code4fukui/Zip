...
test("finds a password from length one upward", () => {
  const data = Zip.compress({ "secret.txt": "secret" }, { password: "ba" });
  assert.deepEqual(findPassword(data, { chars: "ab", maxLength: 2 }), {
    password: "ba",
    attempts: 5,
  });
  assert.deepEqual(findPassword(data, { chars: "a", maxLength: 2 }), {
    password: null,
    attempts: 2,
  });
});
